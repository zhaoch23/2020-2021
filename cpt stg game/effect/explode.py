...
class Explode(Effect):
    ...
    def __init__(self, parent, type_id=['circle','4']): # init
    ...
    def custom_update(self): # animation update
        ...
        self.scale += 0.5
        self.dim += 60
        if self.timer >= 0.5*FRAME_PER_SECOND:
            self.die()
    
    def rescale(self): # animation of explode, usually applied when enemy minion dies
